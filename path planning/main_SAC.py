...
shoplistfile = 'E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\MASAC_new1'  #保存文件数据所在文件的文件名
...
TRAIN_NUM = 1
TEST_EPIOSDE=100
env = RlGame(n=N_Agent,m=M_Enemy,render=RENDER).unwrapped #环境初始化连接
state_number=7
...
def run(env):
    if Switch==0:
            ...
            for k in range(TRAIN_NUM):
                ...
                for episode in range(EP_MAX):
                    ...
                    if episode % 20 == 0 and episode > 200:#保存神经网络参数
                        ...
                        torch.save(save_data, "E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\Path_SAC_actor_L1.pth")
                        save_data = {'net': actors[1].action_net.state_dict(), 'opt': actors[1].optimizer.state_dict()}
                        torch.save(save_data, "E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\Path_SAC_actor_F1.pth")
        ...
        print('SAC测试中...')
        aa = Actor()
        checkpoint_aa = torch.load("E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\Path_SAC_actor_L1.pth")
        aa.action_net.load_state_dict(checkpoint_aa['net'])
        bb = Actor()
        checkpoint_bb = torch.load("E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\Path_SAC_actor_F1.pth")
