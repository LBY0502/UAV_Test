...
class RlGame(gym.Env):
    def __init__(self, n, m, render=False):
        self.hero_num = n  # 己方无人机数量
        self.enemy_num = m  # 敌方无人机数量
        self.obstacle_num = 1  # 障碍物数量
        self.goal_num = 1  # 目标点数量
        self.Render = render  # 是否开启可视化
        self.game_info = {
            'epsoide': 0,
            'hero_win': 0,
            'enemy_win': 0,
        ...
        if self.Render:
            ...
            self.GRAPHICS = tools.load_graphics('E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\assignment\\source\\image')

            self.SOUND = tools.load_sound('E:\\UAV-path-planning-main\\Multi-UAVs path planning\\path planning\\assignment\\source\\music')
